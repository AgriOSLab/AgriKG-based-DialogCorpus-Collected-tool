...
export default {
  ...
  methods:{
    ...
    submitBtn(){
      request.post("/api/collect-dialog-corpus/saveRecords",{'dialog':this.list,'triples':this.graphData.edges, 'username':this.username, "properties":this.showedProperty, "id":this.graphData.id}).then(res=>{
        this.initPage();
        this.canbeSubmit = true
        this.show = false
      })
    ...
    initPage(){
      ...
      request.get(uri+"properties/intentions").then(res=>{
